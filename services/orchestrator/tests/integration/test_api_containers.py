...
class TestContainersEndpoints:
    ...
    def test_create_containers_success(
        ...
        mock_containers = [
            ContainerResponse(
                ...
                name=sample_container_data["name"], internal_port=8080, external_port=32768,
                status="running", cpu_usage="0.0", memory_usage="0m",
                created_at="2024-01-01T00:00:00Z", image_id=sample_container_data["image_id"],
        ...
        # Act
        response = client.post(f"/api/containers/{sample_container_data['image_id']}", json=container_create)
        ...
        assert len(response_data) == 1, f"Expected 1 container, got {len(response_data)}"
        
        # Verify complete container structure
        container = response_data[0]
        required_fields = [
            "id", "container_id", "name", "internal_port",
            "external_port", "status", "cpu_usage", "memory_usage",
            "created_at", "image_id", "user_id"
        ]
    ...
    def test_create_containers_invalid_data(
        self,
        test_user_id: int
    ) -> None:
    ...
    def test_list_containers_success(
        ...
        mock_containers = [
            ContainerResponse(
                ...
                status="running", cpu_usage="0.0", memory_usage="0m",
                created_at="2024-01-01T00:00:00Z", image_id=1, user_id=test_user_id
            ),
            ContainerResponse(
        ...
        response_data = response.json()
        assert isinstance(response_data, list), "Response should be a list"
        assert len(response_data) == 2, f"Expected 2 containers, got {len(response_data)}"
    ...
    @patch('app.api.containers.container_service.start_container')
    def test_start_container_not_found(
        self,
        mock_start: Mock
    ) -> None:
