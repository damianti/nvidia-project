...
class TestBuildContextRoot:
    ...
    @patch(
        "app.services.build_context.DEFAULT_BUILD_CONTEXT_BASE_DIR",
        "/var/lib/orchestrator/build-contexts",
    )
...
class TestPrepareContext:
    ...
    async def test_prepare_context_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            root_dir, context_dir = await prepare_context(
                user_id=1, image_id=42, file=mock_file
            )
    ...
    async def test_prepare_context_with_tar_gz(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Call prepare_context
            root_dir, context_dir = await prepare_context(
                user_id=2, image_id=100, file=mock_file
            )
