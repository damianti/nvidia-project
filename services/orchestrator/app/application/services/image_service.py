...
def _mark_failed_and_commit(
    db: Session, image: Image, *, build_logs: str | None = None
) -> None:
...
async def create_image_from_upload(
    *,
    ...
    try:
        ...
        _assert_app_hostname_unique(
            db, app_hostname=data.app_hostname, user_id=data.user_id
        )
        ...
        build_logs = docker_service.build_image_from_context(
            context_dir, repo, db_image.tag
        )
        ...
        if db_image is not None and db_image.id is not None:
            try:
                ...
                _mark_failed_and_commit(
                    db, db_image, build_logs=detail_text or build_logs
                )
            except Exception:
                db.rollback()
        else:
