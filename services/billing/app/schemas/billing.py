...
class BillingSummaryResponse(BaseModel):
    ...
    total_containers: int = Field(
        ..., description="Total number of containers created for this image"
    )
    total_minutes: int = Field(
        ..., description="Total minutes of usage (sum of all containers)"
    )
    total_cost: float = Field(..., description="Total cost (sum of all containers)")
    active_containers: int = Field(
        ..., description="Number of currently active containers"
    )
    last_activity: Optional[datetime] = Field(
        None, description="Last container start/stop time"
    )
...
class BillingDetailResponse(BaseModel):
    ...
    containers: List[UsageRecordResponse] = Field(
        ..., description="List of all container usage records"
    )
