...
class AuthClient:
    ...
    async def login(self, login_data: dict) -> httpx.Response:
        try:
            ...
            logger.error("auth.proxy.timeout", extra={"timeout_s": self.timeout_s})
            raise
    ...
    async def get_current_user(
        self, authorization_header: str, cookies: Optional[dict] = None
    ) -> httpx.Response:
        try:
    ...
    async def logout(
        self, token: str, cookies: Optional[dict] = None
    ) -> httpx.Response:
        try:
