...
class TestAuthRoutesIntegration:
    ...
    async def test_login_happy_path(
        ...
        assert (
            "set-cookie" in response.headers or "Set-Cookie" in response.headers
    ...
    async def test_login_missing_fields(
        self, client: TestClient, mock_auth_client: Mock
    ) -> None:
    ...
    async def test_signup_duplicate_email(
        ...
        assert "detail" in response_data, "Error response should contain 'detail' field"
        assert (
            "email" in response_data["detail"].lower()
    ...
    async def test_get_current_user_invalid_token(
        self, client: TestClient, mock_auth_client: Mock, mock_error_response_401: Mock
    ) -> None:
        ...
        # Arrange
        mock_auth_client.get_current_user = AsyncMock(
            return_value=mock_error_response_401
        )
    ...
    async def test_get_current_user_server_error(
        ...
        # Arrange
        mock_auth_client.get_current_user = AsyncMock(
            return_value=mock_error_response_500
        )
    ...
    async def test_logout_happy_path(
        ...
        """
        # Arrange
        mock_successful_auth_response.json.return_value = {
        ...
        assert (
            "message" in response_data or "detail" in response_data
