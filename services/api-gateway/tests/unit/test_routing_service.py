...
class TestRoutingService:
    ...
    async def test_get_routing_info_happy_path(
        ...
        route_result = RouteResult(
            ok=True,
            data=sample_routing_info,
            status_code=200
        )
        mock_lb_client.route = AsyncMock(return_value=route_result)
        
        # Act
        result = await routing_service.get_routing_info("testapp.localhost", mock_lb_client)
    ...
    @pytest.mark.asyncio
    async def test_get_routing_info_not_found(
        self,
        mock_lb_client: Mock
    ) -> None:
        ...
        # Act
        result = await routing_service.get_routing_info("nonexistent.localhost", mock_lb_client)
        
        # Assert
        assert result is None, "Result should be None when routing fails"
    
    @pytest.mark.asyncio
    async def test_get_routing_info_no_capacity(
        self,
        mock_lb_client: Mock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_routing_info_server_error(
        self,
        mock_lb_client: Mock
    ) -> None:
    ...
    async def test_resolve_route_from_cache_happy_path(
        ...
        assert result.target_host == sample_cache_entry.target_host, \
            f"Expected target_host {sample_cache_entry.target_host}, got {result.target_host}"
        assert result.target_port == sample_cache_entry.target_port, \
    ...
    def test_create_cache_entry_from_routing_info_happy_path(
        ...
        # Act
        entry = routing_service.create_cache_entry_from_routing_info(sample_routing_info)
        
        # Assert
        assert isinstance(entry, CacheEntry), "Result should be CacheEntry instance"
        assert entry.target_host == sample_routing_info.target_host, \
            f"Expected target_host {sample_routing_info.target_host}, got {entry.target_host}"
        assert entry.target_port == sample_routing_info.target_port, \
    ...
    def test_create_cache_entry_with_default_ttl(
        self
    ) -> None:
