...
class TestProcessContainerStarted:
    ...
    def test_process_container_started_success(
        ...
        # Assert
        mock_get_active.assert_called_once_with(db_session_mock, event_data.container_id)
...
class TestProcessContainerStopped:
    ...
    def test_process_container_stopped_success(
        ...
        """
        # Arrange
        sample_billing_record.start_time = datetime.now(timezone.utc) - timedelta(minutes=30)
    ...
    def test_process_container_stopped_missing_timestamp(
        ...
        with patch('app.services.billing_service.update_usage_record') as mock_update, \
             patch('app.services.billing_service.calculate_duration_minutes') as mock_duration, \
             patch('app.services.billing_service.calculate_cost') as mock_cost:
...
class TestGetBillingSummary:
    ...
    def test_get_billing_summary_success(
        ...
        mock_get_by_image.assert_called_once_with(db_session_mock, test_user_id, test_image_id)
