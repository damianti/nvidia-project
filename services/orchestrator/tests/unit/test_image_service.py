...
class TestCreateImageFromUpload:
    ...
    async def test_create_image_from_upload_success(self, mock_repo, mock_prepare, mock_build):
    ...
    @patch("app.application.services.image_service.prepare_context")
    @patch("app.application.services.image_service.images_repository")
    async def test_create_image_from_upload_duplicate_app_hostname(self, mock_repo, mock_prepare, mock_build):
...
class TestCreateImageFromUploadBuildFailure:
    ...
    @patch("app.application.services.image_service.prepare_context")
    @patch("app.application.services.image_service.images_repository")
    async def test_build_failure(self, mock_repo, mock_prepare, mock_build, db_session_mock):
        
        mock_repo.get_by_app_hostname.return_value = None
        mock_repo.create.side_effect = lambda db, image_obj: setattr(image_obj, "id", 1) or image_obj
        mock_prepare.return_value = ("/tmp/root", "/tmp/root/context")
