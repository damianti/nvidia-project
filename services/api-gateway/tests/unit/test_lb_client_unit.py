...
class TestLoadBalancerClientUnit:
    ...
    async def test_route_happy_path(
        ...
        assert isinstance(result.data, RoutingInfo), \
    ...
    async def test_route_parse_error(
        ...
        assert "parse" in result.message.lower() or \
    ...
    def test_handle_success_missing_fields(
        self,
        lb_client: LoadBalancerClient
    ) -> None:
        ...
        # Arrange
        mock_response = Mock()
        mock_response.json.return_value = {"target_host": "172.19.0.1"}  # Missing other fields
    ...
    def test_handle_unknown_status(
        self,
        lb_client: LoadBalancerClient
    ) -> None:
        ...
        assert "unexpected" in result.message.lower() or \
               str(418) in result.message, \
