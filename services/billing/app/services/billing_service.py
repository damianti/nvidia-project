...
def get_billing_summary(db: Session, user_id: int, image_id: int) -> BillingDetailResponse:
    """
    ...
    for record in records:
        if record.status == BillingStatus.ACTIVE:
            ...
            estimated_duration = calculate_duration_minutes(start_time, datetime.now(timezone.utc))
...
def get_all_billing_summaries(db: Session, user_id: int) -> List[BillingSummaryResponse]:
    """
