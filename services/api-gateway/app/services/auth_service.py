...
async def handle_login(
    login_data: LoginRequest, auth_client: AuthClient, response: Response
):
    ...
    set_cookie = auth_response.headers.get("set-cookie") or auth_response.headers.get(
        "Set-Cookie"
    )
    if set_cookie:
...
async def handle_signup(
    user_data: UserCreate, auth_client: AuthClient, response: Response
):
...
async def handle_get_current_user(
    authorization_header: str, auth_client: AuthClient, request: Request
):
