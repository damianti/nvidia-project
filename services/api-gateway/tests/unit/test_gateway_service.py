...
class TestGatewayService:
    ...
    async def test_handle_route_request_success(
        ...
        with patch('app.services.gateway_service.resolve_route', return_value=sample_cache_entry), \
             patch('app.services.gateway_service.proxy_to_container', return_value=mock_response), \
    ...
    async def test_handle_route_request_not_found(
        ...
        with patch('app.services.gateway_service.resolve_route', return_value=None), \
             patch('app.services.gateway_service.extract_client_ip', return_value="127.0.0.1"):
            ...
            assert "not found" in result.body.decode().lower() or "not available" in result.body.decode().lower()
    ...
    async def test_handle_route_request_path_normalization(
        ...
        with patch('app.services.gateway_service.resolve_route', return_value=sample_cache_entry), \
             patch('app.services.gateway_service.proxy_to_container', return_value=mock_response) as mock_proxy, \
             patch('app.services.gateway_service.extract_client_ip', return_value="127.0.0.1"), \
             patch('app.services.gateway_service.prepare_proxy_headers', return_value={}):
