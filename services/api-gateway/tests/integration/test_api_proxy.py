...
class TestProxyRoutesIntegration:
    ...
    def setup_mocks(
        ...
        app.dependency_overrides[get_orchestrator_client] = lambda: mock_orchestrator_client
    ...
    async def test_apps_route_happy_path(
        ...
        with patch("app.services.gateway_service.resolve_route", return_value=sample_cache_entry), \
             patch("app.services.gateway_service.extract_client_ip", return_value="127.0.0.1"), \
             patch("app.services.gateway_service.prepare_proxy_headers", return_value={}), \
             patch("app.services.gateway_service.proxy_to_container", return_value=mock_proxy_response):
            
            # Act
            response = client.get(
                "/apps/testapp.localhost/",
    ...
    async def test_apps_route_not_found(
        ...
        assert "not found" in response_body.lower() or \
               "no containers" in response_body.lower(), \
    ...
    async def test_apps_route_invalid_hostname(
        ...
        # Act
        response = client.get(
            f"/apps/{invalid_hostname}/",
            headers={"Host": invalid_hostname}
        )
        ...
        response_body = response.text
        assert "invalid" in response_body.lower() or \
               "hostname" in response_body.lower(), \
    ...
    async def test_apps_route_server_error(
        ...
        with patch("app.services.gateway_service.resolve_route", return_value=sample_cache_entry), \
             patch("app.services.gateway_service.extract_client_ip", return_value="127.0.0.1"), \
             patch("app.services.gateway_service.prepare_proxy_headers", return_value={}), \
             patch("app.services.gateway_service.proxy_to_container") as mock_proxy:
    ...
    async def test_upload_image_happy_path(
        ...
        with patch("app.routes.proxy_routes.handle_image_upload", return_value=mock_response):
            # Act
            files = {"file": ("test.tar.gz", b"fake tar content", "application/gzip")}
            response = authenticated_client.post(
                "/api/images",
                data=sample_image_upload_data,
                files=files
            )
    ...
    async def test_proxy_api_happy_path(
        ...
        with patch("app.routes.proxy_routes.handle_orchestrator_proxy", return_value=mock_response):
            # Act
            ...
            response_data = response.json()
            assert isinstance(response_data, list), "Response should be a list of images"
            if len(response_data) > 0:
                assert "id" in response_data[0], "Image object should contain 'id' field"
                assert "name" in response_data[0], "Image object should contain 'name' field"
